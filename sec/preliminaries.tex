...
The \defn{simplex category} is defined to have an object $[n] = \{0, \dots, n\}$ for every $n \in \N$ and a morphism in $\simplex \big([m], [n] \big)$ for each order-preserving function from $[m]$ to $[n]$.
The identity $[n] \to [n]$ is denoted simply as $[n]$.
...
They satisfy the \defn{cosimplicial identities}:
...
\begin{equation}\label{e:canonical factorization}
	\delta_{u_{p}} \dots\, \delta_{u_1}\, \sigma_{v_1} \dots\, \sigma_{v_q}
\end{equation}
for some sets of non-negative integers $U = \{u_1 < \dots < u_{p}\}$ and $V = \{v_1 < \dots < v_{q}\}$.
We simplify notation and write the \defn{canonical factorization} \eqref{e:canonical factorization} simply as $\delta_U \sigma_V$.

A \defn{simplicial set} $X$ is a contravariant functor from the simplex category to the category of sets and a \defn{simplicial map} is a natural transformation between two simplicial sets.
...
X(\sigma_i) = s_i.
\]
Elements in $X_n$ are referred to as \mbox{\defn{$n$-simplices}} with the integer $n$ called its \defn{dimension} and omitted when unspecified.
Simplices in the image of any $s_i$ are termed \defn{degenerate}.
...
We say that a simplex $y$ is a (proper) \defn{face} of a simplex $x$ if $y = d_U(x)$ for some (non-empty) $U$.

For each $n \in \N$, the simplicial set $\simplex^n$, referred to as the \defn{$n^\th$ standard simplicial set}, is defined by
\[
...
The functor of \defn{normalized chains} (with $\F$-coefficients) $\chains \colon \sSet \to \Ch$ is defined on objects as follows:
\[
...
The functor of \defn{normalized cochains} $\cochains$ is defined by composing $\chains$ with the linear duality functor $\Hom(-, \F)$.
