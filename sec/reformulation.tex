...
This section provides a categorical perspective on (semi-)simplicial \mbox{cup-$i$} constructions.
We begin by presenting their linear dual formulation.
...
In other words, the linear duality functor induces a bijection between \mbox{cup-$i$} product structures on $C^\vee$ and \mbox{cup-$i$} coproduct structures on $C$.
...
\begin{proof}
	...
	\begin{align*}
		...
		d_{\bd U}[n].
	\end{align*}
	The verification of compatibility with cofaces and codegeneracies is straightforward and left to the reader.
\end{proof}
...
\end{itemize}
To see this we notice that there is $k \in V_\lambda \cap W_\lambda$ for one of the summands of $\triangle_i [n]$ if and only the image of this summand under $\Psi^{\ot 2}$ is
\[
...
\begin{proof}
	...
	To verify that it is simplicial, we consider \cref{eq:codegeneracies} and an arbitrary basis element $U^0 \ot U^1$ appearing as a summand in $\canonical_i[n]$.
	Let $U = U^0 \union U^1$.
...
We conclude with a few special cases of particular importance.
For any $n \in \N$,
\[
