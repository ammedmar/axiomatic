...
\[
W \ot_{\Ftwo[\Sym_2]} \cochains(\simplex^n)^{\ot 2} \to \cochains(\simplex^n).
\]
...
\]
with $\triangle_{-1} = 0$.
Each natural transformation $\triangle_i$ in \eqref{eq:natural_linear_maps} is completely determined by the set of tensor chains
\[
...
\bd U =
\begin{cases}
	\displaystyle\sum_{\mathclap{\bar{u} \notin U}} \bar{u}.U, & m>0, \\
	0, & m=0,
\end{cases}
\]
...
\begin{lemma}\label{st:chains_functor_equivalence}
	The function
	\[
	\begin{tikzcd}[column sep=small,row sep=0]
...
\begin{proof}
	...
	For $m=0$, both sides have zero boundary.
	For $m>0$, using that for $u \leq j$ we have $d_jd_u = d_ud_{j+1}$, we have
	\begin{align*}
		\bd d_U[n] &=
		...
		d_{\bd U}[n].
	\end{align*}
	The compatibility with cofaces and codegeneracies follows directly from their definitions.
\end{proof}
