...
We start by presenting a linear dual description of cup-\(i\) products.
\anibal{It is more categorical, but also dual in two ways: linear pre-dual to cochains and using Alexander duality/Poincar'e duality in the n-simplex}
...
\anibal{Make the statement below into a Lemma and fix the hyperlinks in the text as this will introduce two targets for the old ss:axiomsrevisited}
...
\begin{proof}
	...
	To see it is simplicial, \anibal{Here we could say that this follows from the irred + semi-simp implies simp but we also give a direct proof.} we consider \cref{eq:codegeneracies} and an arbitrary basis element $U^0 \ot U^1$ appearing as a summand in $\canonical_i[n]$.
	Let $U = U^0 \union U^1$.
