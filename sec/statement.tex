...
We denote by $\cochains(X)$ the (normalized) cochains of a simplicial or semi-simplicial set $X$ with coefficients in $\Ftwo$, graded homologically and concentrated in non-positive degrees.
...
Unpacking this definition, we obtain the following identity for any $i \in \N$ and $\alpha, \beta \in A$:
\[
...
Setting $i = 1$ implies that the algebra structure induced on the homology $HA$ of $A$ is commutative, with the product $\cup_1$ encoding this relation at the chain level.
...
A \textbf{(semi-)simplicial \mbox{cup-$i$} construction} is a \mbox{cup-$i$} product structure on $\cochains(X)$, assigned to each (semi-)simplicial set $X$, that is natural with respect to (semi-)simplicial maps.
...
Note that a simplicial \mbox{cup-$i$} construction is a semi-simplicial one satisfying additional constraints, namely compatibility with degeneracy maps.

\subsection{Axioms}

The first axiom mentioned in the introduction, naturality, is explicitly incorporated into our definition of a (semi-)simplicial \mbox{cup-$i$} construction, while the second, minimality, is reflected in the use of $W$ rather than an arbitrary free resolution of $\F$.
...
\begin{itemize}
	...
	\]
	whenever $y$ is a proper face of $x$ and $y^{(1)}$, $y^{(2)}$ are faces of $y$.

	\item \textbf{Free} if, for any two simplices $x, x'$ in $X$,
	\[
...
\medskip In \cref{s:others,s:operads} we will show that all semi-simplicial \mbox{cup-$i$} constructions defined in the literature represent the same isomorphism class, and in fact, they are all simplicial.
...
For any $n$-simplex $x$ and subset $U = \{u_1 < \dots < u_r\} \subseteq \{0,\dots,n\}$, we write $d_U(x)$ for the composition of face maps $d_{u_1} \cdots d_{u_r}(x)$, with $d_{\emptyset}(x) = x$.
