...
The main text identifies $\widehat{\cP}(\simplex^n)$ with the Koszul complex of $\omega = x_0 + \dots + x_n$ in the exterior algebra on the vertices, the partition chains with the image of its coproduct $\Delta$, and the span of the reducible basis elements with the ideal generated by the elements $x_k \ot x_k$, hence a subcomplex.
We write $\mu$ for the multiplication of $\widehat{\cP}(\simplex^n)$.
...
As recalled in the main text, the algebra $\widehat{\cP}(\simplex^n)$ is the group ring $\F[C_2^{\, n+1}]$ under $t_u = 1 + x_u$, but the partition coproduct is the primitively generated structure, the Cartier dual of the group-ring one.
Several threads deserve a fuller treatment, which we leave for future work.
\begin{enumerate}
	\item The primitively generated exterior Hopf algebra is self-dual: the monomial basis is dual to itself, and under the resulting identification of $\widehat{\cP}(\simplex^n)$ with its dual the multiplication $\mu$ and the partition coproduct $\Delta$ are adjoint.
	The interplay between the ideal spanned by the reducible basis elements and the subcomplex spanned by the partition chains, and hence the relation between the reducible quotient and \cref{l:partition_chains}, should be understood in these terms, as should the Poincar\'e--Lefschetz duality of the deleted product sketched above.
	We caution that the pairing $\coeff{x_U\, x_{U'}}{x_{\set{0,\dots,n}}}$ defined by the top class -- the one implementing the passage between removed sets and vertex sets -- is a different one, with respect to which $\mu$ and $\Delta$ are not adjoint.
